...
var openFile = function (event) {
	...
	if (filetype == "text/plain") {
		...
		reader.onload = function () {
			...
			var node = document.getElementById('teleprompter000');
			node.innerText = text;
...
$(function () {
	...
	if ($.cookie('teleprompter_text_color')) {
		...
		$('#teleprompter000').css('color', textColor);
	}
	if ($.cookie('teleprompter_background_color')) {
		...
		$('#teleprompter000').css('background-color', backgroundColor);
	}
	else {
		clean_teleprompter();
	}
	...
	$('#teleprompter000').keyup(update_teleprompter);
	...
	$('#text-color').change(function () {
		var color = $(this).val();
		$('#teleprompter000').css('color', color);
		$.cookie('teleprompter_text_color', color);
	});
	$('#background-color').change(function () {
		var color = $(this).val();
		$('#teleprompter000').css('background-color', color);
	...
	$('.button.play').click(function () {
		if ($(this).hasClass('icon-play')) {
			...
			SR.pause();
			if ($('.marker').hasClass('icon-play')) {
				$('.marker').removeClass('icon-play').addClass('icon-pause');
				window.timer.stopTimer();
			} else {
				$('.marker').removeClass('icon-pause').addClass('icon-play');
...
function navigate(evt) {
	...
	if (typeof evt.target.id == 'undefined' || evt.target.id == 'teleprompter000') {
		return;
	}
...
function start_teleprompter() {
	$('#teleprompter000').attr('contenteditable', false);
...
function stop_teleprompter() {
	SR.stop();
	clearTimeout(scrollDelay);
	$('#teleprompter000').attr('contenteditable', true);
...
function update_teleprompter() {
	$.cookie('teleprompter_text', $('#teleprompter000').html());
}

// Clean Teleprompter
function clean_teleprompter() {
	var text = $('#teleprompter000').html();
